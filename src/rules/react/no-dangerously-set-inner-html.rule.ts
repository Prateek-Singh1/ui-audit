...
import { findNodesByKinds, getJsxAttributeName, nodeLocation } from './jsx-helpers.js';
...
export class NoDangerouslySetInnerHtmlRule extends BaseRule {
  constructor() {
    super(
      defineRuleMetadata({
        ...
        description: 'Disallows the dangerouslySetInnerHTML prop to reduce XSS risk.',
        category: RuleCategory.React,
        severity: RuleSeverity.Error,
        recommended: true,
        enabledByDefault: true,
        documentationUrl:
          'https://github.com/Prateek-Singh1/ui-audit/blob/main/docs/rules/react/no-dangerously-set-inner-html.md',
      }),
    );
  }
  ...
  protected run(context: RuleContext): RuleResult {
    const findings = findNodesByKinds(context.ast.root, ['JsxAttribute'])
      .filter((attribute) => getJsxAttributeName(context.ast, attribute) === 'dangerouslySetInnerHTML')
      .map((attribute) =>
        createFinding({
          context,
          ruleName: this.name,
          message: 'Avoid dangerouslySetInnerHTML because it can introduce XSS vulnerabilities.',
