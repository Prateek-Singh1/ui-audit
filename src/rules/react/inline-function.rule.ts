  ...
  constructor() {
    super(
      defineRuleMetadata({
        ...
        documentationUrl:
          "https://github.com/Prateek-Singh1/ui-audit/blob/main/docs/rules/react.md#reactinline-function-prop",
      }),
    );
  }
  ...
  protected run(context: RuleContext): RuleResult {
    const findings = findAstNodes(
      ...
      (node) =>
        ...
        this.isInlineFunctionProp(context, node),
    ).map((node) =>
      createFinding({
        context,
        ruleName: this.name,
