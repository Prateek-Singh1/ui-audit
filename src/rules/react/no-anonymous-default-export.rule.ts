...
import { findNodesByKinds, firstExpressionChild, nodeLocation } from './jsx-helpers.js';
...
export class NoAnonymousDefaultExportRule extends BaseRule {
  constructor() {
    super(
      defineRuleMetadata({
        ...
        documentationUrl:
          'https://github.com/Prateek-Singh1/ui-audit/blob/main/docs/rules/react/no-anonymous-default-export.md',
      }),
    );
  }
  ...
  private isAnonymousDefault(context: RuleContext, node: NormalizedAstNode): boolean {
    if (node.kind === 'ExportAssignment') {
      const expression = firstExpressionChild(node);
      return expression !== undefined && ANONYMOUS_EXPRESSION_KINDS.has(expression.kind);
    }
