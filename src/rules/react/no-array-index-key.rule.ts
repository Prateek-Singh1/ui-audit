...
export class NoArrayIndexKeyRule extends BaseRule {
  constructor() {
    super(
      defineRuleMetadata({
        ...
        documentationUrl:
          "https://github.com/Prateek-Singh1/ui-audit/blob/main/docs/rules/react.md#reactno-array-index-key",
      }),
    );
  }
  ...
  protected run(context: RuleContext): RuleResult {
    const findings = findNodesByKinds(context.ast.root, ["JsxAttribute"])
      .filter(
        (attribute) => getJsxAttributeName(context.ast, attribute) === "key",
      )
