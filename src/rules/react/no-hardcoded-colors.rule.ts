...
export class NoHardcodedColorsRule extends BaseRule {
  constructor() {
    super(
      defineRuleMetadata({
        ...
        description: 'Disallows hardcoded color values in inline style objects.',
        category: RuleCategory.React,
        severity: RuleSeverity.Info,
        recommended: false,
        enabledByDefault: true,
        documentationUrl:
          'https://github.com/Prateek-Singh1/ui-audit/blob/main/docs/rules/react/no-hardcoded-colors.md',
      }),
    );
  }
  ...
  protected run(context: RuleContext): RuleResult {
    const styleObjects = findNodesByKinds(context.ast.root, ['JsxAttribute'])
      .filter((attribute) => getJsxAttributeName(context.ast, attribute) === 'style')
      .map((attribute) => getJsxAttributeValue(attribute))
      .filter((value): value is NonNullable<typeof value> => value?.kind === 'ObjectLiteralExpression');

    const findings = styleObjects
      .flatMap((object) => findNodesByKinds(object, ['StringLiteral']))
      .filter((literal) => isHardcodedColor(getAstNodeText(context.ast, literal)))
      .map((literal) =>
        createFinding({
          context,
          ruleName: this.name,
...
const isHardcodedColor = (rawLiteral: string): boolean => {
  ...
  return HEX_COLOR.test(value) || FUNCTIONAL_COLOR.test(value) || NAMED_COLORS.has(value.toLowerCase());
};
