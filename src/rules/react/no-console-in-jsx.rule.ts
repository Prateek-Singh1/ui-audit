...
export class NoConsoleInJsxRule extends BaseRule {
  constructor() {
    super(
      defineRuleMetadata({
        ...
        description: 'Disallows console calls inside JSX expression containers.',
        category: RuleCategory.React,
        severity: RuleSeverity.Info,
        recommended: false,
        enabledByDefault: true,
        documentationUrl:
          'https://github.com/Prateek-Singh1/ui-audit/blob/main/docs/rules/react/no-console-in-jsx.md',
      }),
    );
  }
  ...
  protected run(context: RuleContext): RuleResult {
    ...
    for (const expression of findNodesByKinds(context.ast.root, ['JsxExpression'])) {
      for (const call of findNodesByKinds(expression, ['CallExpression'])) {
        ...
        byOffset.set(
          ...
          createFinding({
            ...
            suggestion: 'Remove the console call or move it out of the render output.',
          }),
        );
      }
    }
