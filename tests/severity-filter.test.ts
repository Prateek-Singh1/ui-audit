...
const makeFinding = (severity: Finding['severity'], ruleId: string): Finding => ({
  ruleId,
  severity,
...
describe('parseSeveritySelection', () => {
  it('selects every severity by default', () => {
    ...
    expect(selection.severities).toEqual(['critical', 'error', 'warning', 'info']);
  });
...
describe('filterFindingsBySeverity', () => {
  ...
  it('keeps only matching findings and counts the hidden ones, preserving order', () => {
    const { visible, hidden } = filterFindingsBySeverity(findings, ['error', 'warning']);

    expect(visible.map((finding) => finding.ruleId)).toEqual(['b/error', 'c/warning']);
...
describe('JSON reporter severity metadata', () => {
  ...
  it('omits selectedSeverities and preserves the schema when not filtering', () => {
    ...
    expect(report.metadata).toEqual({ tool: 'ui-audit', reporter: 'json', schemaVersion: 1 });
  });
...
describe('runAuditCommand --severity', () => {
  it('reports only the single selected severity', async () => {
    ...
    const errorCount = all.result.findings.filter((f) => f.severity === 'error').length;
    expect(errorCount).toBeGreaterThan(0);

    const outcome = await runAuditCommand({ path: root, cwd: root, severity: 'error' });
    ...
    expect(outcome.result.hiddenFindings).toBe(all.result.findings.length - errorCount);
  ...
  it('reports multiple selected severities and leaves execution stats intact', async () => {
    ...
    const all = await runAuditCommand({ path: root, cwd: root });
    const outcome = await runAuditCommand({ path: root, cwd: root, severity: 'info,error' });

    expect([...severitiesOf(outcome.result.findings)].every((s) => s === 'info' || s === 'error')).toBe(true);
  ...
  it('rejects an unknown severity with a friendly validation error', async () => {
    ...
    ).rejects.toThrow(/Unknown severity "fatal"\. Supported severities: info, warning, error, critical\./);
  });
  ...
  it('produces deterministic output regardless of token order', async () => {
    ...
    const first = await runAuditCommand({ path: root, cwd: root, severity: 'error,info' });
    const second = await runAuditCommand({ path: root, cwd: root, severity: 'info,error' });

    expect(first.result.selectedSeverities).toEqual(second.result.selectedSeverities);
