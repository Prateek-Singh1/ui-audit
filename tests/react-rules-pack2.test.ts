...
describe('react/no-unused-use-state', () => {
  ...
  it('flags an unused value or setter', async () => {
    expect(await count(rule, 'const C=()=>{const [v,setV]=useState(0); setV(1); return <div/>;};')).toBe(1);
    expect(await count(rule, 'const C=()=>{const [v,setV]=useState(0); return <div>{v}</div>;};')).toBe(1);
  });

  it('does not flag fully used state', async () => {
    expect(
      await count(rule, 'const C=()=>{const [v,setV]=useState(0); return <div onClick={()=>setV(1)}>{v}</div>;};'),
    ).toBe(0);
  });

  it('respects underscore-prefixed intentionally unused names', async () => {
    expect(await count(rule, 'const C=()=>{const [_v,setV]=useState(0); setV(1); return <div/>;};')).toBe(0);
  });

  it('supports TypeScript generic useState (TSX)', async () => {
    expect(await count(rule, 'const C=()=>{const [v,setV]=useState<number>(0); setV(1); return <div/>;};')).toBe(1);
  });
});
...
describe('react/no-direct-state-mutation', () => {
  ...
  it('is an error rule and flags nested assignment and mutating methods', async () => {
    expect(rule.severity).toBe('error');
    expect(await count(rule, 'state.user.name = "John";')).toBe(1);
    expect(await count(rule, 'items.push(1);')).toBe(1);
  });

  it('ignores this.* and ref.current assignments', async () => {
    expect(await count(rule, 'this.value = 1;')).toBe(0);
    expect(await count(rule, 'ref.current = 1;')).toBe(0);
  });

  it('does not flag non-mutating array methods', async () => {
    expect(await count(rule, 'const next = items.map((x) => x + 1);')).toBe(0);
  });
});
...
describe('react/no-multiple-state-updates', () => {
  ...
  it('treats separate blocks independently (nested)', async () => {
    expect(await count(rule, 'function h(){ if (x) { setA(1); } else { setB(2); } }')).toBe(0);
  });
});
...
describe('react/use-effect-cleanup', () => {
  ...
  it('flags subscriptions/timers without cleanup', async () => {
    expect(await count(rule, 'useEffect(()=>{ window.addEventListener("x", h); }, []);')).toBe(1);
    expect(await count(rule, 'useEffect(()=>{ const id = setInterval(tick, 1000); }, []);')).toBe(1);
  });
...
describe('react/no-large-jsx-tree', () => {
  ...
  const tree = (n: number): string => `const C=()=><div>${'<span/>'.repeat(n)}</div>;`;
...
describe('react/prefer-use-callback', () => {
  ...
  it('flags inline callbacks passed to components', async () => {
    expect(await count(rule, 'const C=()=><Child onClick={()=>go()}/>;')).toBe(1);
  });

  it('does not flag inline callbacks on DOM elements', async () => {
    expect(await count(rule, 'const C=()=><button onClick={()=>go()}/>;')).toBe(0);
  });
});
...
describe('react/prefer-use-memo', () => {
  ...
  it('flags array transforms and object/array creation during render', async () => {
    expect(await count(rule, 'const C=()=>{const rows=items.map((x)=>x*2); return <div>{rows}</div>;};')).toBe(1);
    expect(await count(rule, 'const C=()=>{const style={a:1}; return <div>{JSON.stringify(style)}</div>;};')).toBe(1);
  });

  it('does not flag cheap values', async () => {
    expect(await count(rule, 'const C=()=>{const n=5; return <div>{n}</div>;};')).toBe(0);
  });

  it('does not flag values created outside components', async () => {
    expect(await count(rule, 'const config={a:1}; export function util(){ return config; }')).toBe(0);
  });
});
