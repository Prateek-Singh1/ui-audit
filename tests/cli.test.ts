...
describe('computeExitCode', () => {
  it('defaults to failing only on error or higher', () => {
    expect(computeExitCode([finding('info'), finding('warning')], { strict: false })).toBe(0);
  ...
  it('honors an explicit fail-on-severity threshold', () => {
    expect(computeExitCode([finding('warning')], { strict: false, failOnSeverity: 'warning' })).toBe(1);
    expect(computeExitCode([finding('info')], { strict: false, failOnSeverity: 'warning' })).toBe(0);
  });
});
...
describe('runAuditCommand', () => {
  ...
  it('exits 1 under --strict when any finding is produced', async () => {
    ...
    const outcome = await runAuditCommand({ path: root, cwd: root, strict: true });
  ...
  it('renders machine-readable JSON with --reporter json', async () => {
    ...
    const outcome = await runAuditCommand({ path: root, cwd: root, reporter: 'json' });
  ...
  it('honors severity overrides from a discovered config file', async () => {
    ...
    const inlineStyle = outcome.result.findings.find((f) => f.ruleId === 'react/inline-style');
  ...
  it('loads an explicit config path via --config', async () => {
    ...
    expect(outcome.result.findings.some((f) => f.ruleId === 'react/inline-style')).toBe(false);
  });

  it('rejects an unknown reporter', async () => {
    const root = await createTempProject();

    await expect(runAuditCommand({ path: root, cwd: root, reporter: 'html' })).rejects.toThrow(
      AuditCommandError,
    );
  });
